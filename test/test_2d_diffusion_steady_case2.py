...
from src._source import PIELM_HeatGen
import logging 
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

ansys_data = pd.read_csv('data/steady_state/case2.xls', sep='\t', encoding='ISO-8859-1')
...
T_scale = ansys_data['temp'].max() - ansys_data['temp'].min()
model = PIELM_HeatGen(hidden_nodes=2000)
model.train(nx=40, ny=40, x_max=x_max, y_max=y_max, T_left=700.0, T_other=500, k=50., q=1e9, T_scale=T_scale)
...
plt.tight_layout()
plt.savefig('plots/pielm_comparison_steady_case2.png', dpi=150, bbox_inches='tight')
plt.show()
logger.info("\nSaved plots/pielm_comparison_steady_case2.png")