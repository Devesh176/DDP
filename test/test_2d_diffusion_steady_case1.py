...
from src._source import PIELM2DDiffusion
import logging 
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# Load and CLEAN ANSYS data
ansys_data = pd.read_csv('data/steady_state/case1.xls', sep='\t', encoding='ISO-8859-1')
...
model = PIELM2DDiffusion(hidden_nodes=2500)
model.train(nx=50, ny=50, x_max=x_max, y_max=y_max, T_left=700.0, T_other=500.0)
...
# Normalised error for comparison with paper targets (1e-4 to 1e-6)
...
plt.tight_layout()
plt.savefig('plots/pielm_comparison_steady_case1.png', dpi=150, bbox_inches='tight')
plt.show()
logger.info("\nSaved plots/pielm_comparison_steady_case1.png")