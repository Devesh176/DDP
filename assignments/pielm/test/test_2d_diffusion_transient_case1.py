...
from src._source import PIELM2DDiffusion_transient
import logging 
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

data_dir = Path('data/transient_state/case1')
df = []
for file in data_dir.glob('*.xls'):
    ...
    ansys_data['t'] = file.stem.split(' ')[-1]  # extract time from filename
...
model = PIELM2DDiffusion_transient(hidden_nodes=5390)
T_scale = df['temp'].max() - df['temp'].min()
model.train(nx=40, ny=40, x_max=x_max, y_max=y_max, t_max=t_max, T_scale=T_scale, T_left=700.0, T_other=500.0, rho=4850.)

X_pred = df[['x', 'y', 't']].values
per_batch = 10000
T_pred = np.zeros(len(X_pred))
# batch wise prediction to avoid memory issue 
...
for t in sorted(df['t'].unique()):
    ...
    plt.tight_layout()
    plt.savefig(f'plots/pielm_comparison_transient_case1_{t}.png', dpi=150, bbox_inches='tight')
    plt.show()
    logger.info(f"\nSaved plots/pielm_comparison_transient_case1_{t}.png")