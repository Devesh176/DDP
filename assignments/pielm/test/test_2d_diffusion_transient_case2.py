...
from src._source import PIELM_HeatGen_Transient
import logging 
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

data_dir = Path('data/transient_state/case2')
df = []
for file in data_dir.glob('*.xls'):
    ...
    ansys_data = ansys_data[['x', 'y', 't', 'temp']]  # only keep relevant column
...
model = PIELM_HeatGen_Transient(hidden_nodes=5390)
T_scale = df['temp'].max() - df['temp'].min()
model.train(nx=20, ny=20, x_max=x_max, y_max=y_max, t_max=t_max, T_left=700.0, T_other=500.0, k=50., q=1e9,
              T_scale=T_scale, rho=7850., cp=434., T_initial=500.,seed=42)
...
for t in sorted(df['t'].unique()):
    ...
    plt.tight_layout()
    plt.savefig(f'plots/pielm_comparison_transient_case2_{t}.png', dpi=150, bbox_inches='tight')
    plt.show()
    logger.info(f"\nSaved plots/pielm_comparison_transient_case2_{t}.png")